            ...
            Twitte12.findOne({ 'discord': message.author.id }, (err1, DataBOI1) => {
                if (DataBOI1) {
                   if (DataBOI1.discord == message.author.id) {
                        ...
                        if (!reason == ! null) return message.channel.send(`Failed to change appSecret.The appSecret Cant be null.`);
                        Twitte12.collection.updateOne({ "discord": message.author.id }, { $set: { "appSecret":  reason} })
                        message.reply("Changed..appSecret")
                   }else{
